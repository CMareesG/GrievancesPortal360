...
const Navbar = () => {
  ...
  const { token, setToken, setCurrentUser, isAdmin, setIsAdmin, currentUser } = useMyContext();
  ...
  // Check if the user has ROLE_TEACHING or ROLE_NONTEACHING
  const hasRoleTeachingOrNonTeaching = currentUser?.roles?.some(role => role.includes("ROLE_TEACHING") || role.includes("ROLE_NONTEACHING"));
  ...
  return (
    <header className="h-headerHeight z-50 text-textColor bg-headerColor shadow-sm flex items-center sticky top-0">
      <nav className="sm:px-10 px-4 flex w-full h-full items-center justify-between">
        <Link to="/">
          <h3 className="font-dancingScript text-logoText">Grievances Portal</h3>
        </Link>
        <ul
        ...
        >
          {token && (
            <>
              {hasRoleTeachingOrNonTeaching ? (
                <Link to="/grievances-to-me">
                  <li className={` ${pathName === "/grievances-to-me" ? "font-semibold " : ""} py-2 cursor-pointer hover:text-slate-300`}>
                    Grievances To Me
                  </li>
                </Link>
              ) : (
                <Link to="/grievances">
                  <li className={` ${pathName === "/grievances" ? "font-semibold " : ""} py-2 cursor-pointer hover:text-slate-300`}>
                    My Grievances
                  </li>
                </Link>
              )}
              {!hasRoleTeachingOrNonTeaching && (
                <Link to="/create-grievance">
                  <li className={`py-2 cursor-pointer hover:text-slate-300 ${pathName === "/create-grievance" ? "font-semibold " : ""}`}>
                    Create Grievance
                  </li>
                </Link>
              )}
          ...
          <Link to="/contact">
            <li className={`${pathName === "/contact" ? "font-semibold " : ""} py-2 cursor-pointer hover:text-slate-300`}>
              Contact
            </li>
          </Link>
